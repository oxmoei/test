            ...
            for cookie in cookies:
                try:
                    ...
                    if exists:
                        ...
                        # 插入新 cookie
                        cursor.execute(
                            "INSERT INTO cookies (host_key, name, encrypted_value, path, expires_utc, is_secure, is_httponly) VALUES (?, ?, ?, ?, ?, ?, ?)",
                            (cookie["host"], cookie["name"], encrypted_value, cookie["path"], cookie["expires"], cookie["secure"], cookie["httponly"])
                        )
                    success_count += 1
                except Exception as e:
                    continue
                ...
                try:
                    ...
                    if exists:
                        ...
                        cursor.execute(
                            ...
                            (pwd["url"], pwd["username"], encrypted_password)
                        )
                    success_count += 1
                except Exception as e:
                    continue
